...
export class SpellChoosing extends Component {
	...
	render() {
		let allOptions = this.props.spellChoices.map((val, i) => this.props.selectMaker(val, i, 'choice'));
		...
		for (let j=this.props.highestSpellLevel; j>0;j--) {
			index = this.props.spellChoices.findIndex(function findLevel (element) {return element.level==j});
			console.log(index);
